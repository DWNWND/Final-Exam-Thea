...
export default function ListBookings({ bookings, maxVenuesShown, isLoading, setIsLoading }) {
  return (
      ...
      {bookings && bookings.length >= 1 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3 gap-4 ">
          {bookings.slice(0, maxVenuesShown).map((booking) => (
            <VenueCard venue={booking.venue} key={booking.id}  myBookings={true} isLoading={isLoading} setIsLoading={setIsLoading}/>
          ))}
        </div>
      )}

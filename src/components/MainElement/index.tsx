...
export default function MainElement({ children, tail = "", noPadding = false }: MainElementProps) {
  ...
  return (
    <div className="relative">
      ...
      <main className={`${!noPadding && "pt-24 p-4 pb-12 md:px-8"} transition duration-1000 ease-in-out min-h-screen bg-background ${tail} ${isMenuOpen ? "blur-sm" : "blur-none"}`}>{children}</main>
    </div>
  );
}

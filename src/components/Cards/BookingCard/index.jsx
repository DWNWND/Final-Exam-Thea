...
export default function BookingCard({ booking, bookingId, bookingDates = null, loading, setSelectedBooking, setCancellationModal = () => {} }) {
    ...
    <>
      ...
      {booking && (
        <div key={booking.id} className="rounded-lg shadow-sm bg-white hover:shadow-lg transition duration-300 ease-in-out relative flex flex-col">
          <Link to={"/venue/" + booking.id} className="h-48 w-full absolute z-30 rounded-lg"></Link>
          ...
          <div className="relative">
            <ArrowRightBtn href={"/venue/" + booking.id} booking={true} tailw={`z-30 ${inactiveBooking && "opacity-60"}`} />
            ...
            {checkOutToday ? (
              ...
              <>
                {inactiveBooking ? (
                  <>
                    <p className="absolute w-full text-center bottom-20 text-3xl font-bold text-white z-20 uppercase">Inactive</p>
                    <p className="absolute w-full text-center bottom-12 text-lg font-bold text-white z-20 italic">
